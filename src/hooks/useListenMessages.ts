...
import { allMessageAdd, messageAdd } from "../utils/redux/slice/messageSlice";
...
const useListenMessages = () => {
  ...
  useEffect(() => {
    socket?.on("responsedeleteEveryOne", (messageId) => {
      const updatedMessages = messageData.messageData.filter(
        (item) => item._id != messageId
      );

      dispatch(allMessageAdd({ data: updatedMessages }));
    });
    ...
    useEffect(() => {
        ...
        socket?.on("receiveMsg", (message) => {
          ...
          dispatch(messageAdd({data:message}))
  ...
  }, [socket,dispatch,messageData.messageData]);
