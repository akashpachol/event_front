...
const Home: React.FC = () => {
  ...
  useEffect(() => {
    if (localStream && !peerConnection && user.userId)
      initializePeerConnection(user.userId, typeOfCall);
  }, [localStream]);
  ...
  useEffect(() => {
    if (remoteStream && peerConnection) navigate(`/viedoCall`);
  ...
  useEffect(() => {
    if (availableCalls.length > 0) {
      setCurrentCall(availableCalls[0]); // Automatically handle the first available call
      setOpen(true);
    } else {
      setOpen(false);
    }
