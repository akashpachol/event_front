...
if(chat.data&&chat.data.users){
  ...
  useEffect(() => {
    const shareVideoAsync = async () => {
      ...
      socket?.emit("newOffer", offer,user.userId,reciver);

      setOfferCreated(true);
      }
    };
    if (!offerCreated ) shareVideoAsync();
  }, [offerCreated,peerConnection,user.userId]);
  ...
  useEffect(() => {
    const addOfferAndCreateAnswerAsync = async () => {
      if(peerConnection && offerData && peerConnection.signalingState === "stable"){
        ...
        copyOfferData.answerUserName = user.userId;
        setAnswerCreated(true);
  
        await socket?.emitWithAck("newAnswer", copyOfferData,user.userId);
      }
  ...
  }, [answerCreated, offerData, peerConnection,user.userId]);
