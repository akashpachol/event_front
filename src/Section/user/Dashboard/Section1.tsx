...
import { eventCards } from "../../../utils/Contents";
...
const Section1: React.FC = () => {
  ...
  const handleNext = () => {
    setStartIndex((prevIndex) =>
      Math.min(prevIndex + 1, eventCards.length - cardsToShow)
    );
  };
  ...
  const isNextDisabled = startIndex + cardsToShow >= eventCards.length;
    ...
    <section className="my-12 flex items-center justify-center">
      ...
      <div className="flex flex-wrap overflow-hidden w-full justify-between">
        {eventCards
          ...
          .map((eventCardData) => (
            <IconCard key={eventCardData.name} {...eventCardData} />
          ))}
      </div>
      <button
        onClick={handleNext}
