...
import {
  ...
  getMessage,
  postMessage,
} from "../../../service/api/vender/apiMethod";
...
const Chat: React.FC = () => {
    ...
    if (chat.data) {
      if (chat.data.chatName == "") {
        ...
        const selectedUser = chat.data.users?.find(
          (userData) => userData._id !== vender.venderId
        );
        if (selectedUser) {
  ...
  const handleMessageDeleteForMe = async (id: string) => {
    try {
      if (!vender.venderId) return;
      const response = await deleteForMe(id, vender.venderId);
      if (response.status === "success") {
        setApi(!api);
      }
    } catch (error) {
                  ...
                  {messages?.map((value: message) => (
                    <>
                      ...
                      {value.sender._id === vender.venderId ? (
                        value.deletedBy === vender.venderId ? (
                          ""
                        ) : (
                          <div
                              ...
                              >
                                <ul>
                                  ...
                                  <li
                                    className="p-2"
                                    onClick={() =>
                                      handleMessageDeleteForMe(value._id)
                                    }
                                  >
                                    Delete for me
                                  </li>
