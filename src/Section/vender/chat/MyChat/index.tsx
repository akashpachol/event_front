...
import {getAllChats,postChat,searchData,} from "../../../../service/api/vender/apiMethod";
...
const MyChat: React.FC<propMychat> = ({ showChat }) => {
  ...
  const getDetails = async () => {
    try {
      ...
      const response = await getAllChats(vender.venderId);
  ...
  const handleSearch = async (query: string) => {
    try {
      if (!query || !vender.venderId) {
        setSearchResult([]);
        return;
      }

      const response = await searchData(query, "manager", vender.venderId);
      if (response && Array.isArray(response.data)) {
  ...
  const handleCreateChat = async (managerValue: userDataTypes) => {
    try {
      ...
      const response = await postChat(vender.venderId, managerValue._id);
